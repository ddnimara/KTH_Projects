...
def main():
    ...
    width = 0.05
    mean = 0
    mu = np.random.normal(0, sigma, n_nodes)
    ...
    for epoch in range(epochs):
        ...
        mu, update_index = rbf_network.competitive_learning(new_x_train[0], mu, eta, update_index,
                                                            method="normal")  # Or "leaky_learning"
        ...
        for i in range(new_x_train.shape[0]):
            d_weights = rbf_network.error_function_minimizer(new_y_train[i, :], weights, transfer_f_train[i, :],
                                                             n_samples, n_nodes, eta)
            weights += d_weights

        # Predict values
        y_train_predict, y_predict = rbf_network.predict_values(y_test, new_y_train, weights, transfer_f_test,
                                                                transfer_f_train)
